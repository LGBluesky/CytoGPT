...
CONV_VISION_Vicuna0 = Conversation(
    # system="",
    system="You are a cytopathologist. Please analyze cervical cytology images according to the 2014 Bethesda System (TBS)."
           "Give the following image: <Img>ImageHere</Img>. "
           "You will be able to see the image once I provide it to you. Please answer my questions.",
    roles=("Human: ", "Assistant: "),
    messages=[],
    offset=2,
...
class Chat:
    ...
    def ask(self, text, conv, pred_txt):
        if len(conv.messages) > 0 and conv.messages[-1][0] == conv.roles[0] \
                and conv.messages[-1][1][-6:] == '</Img>':  # last message is image.
            conv.messages[-1][1] = ' '.join([conv.messages[-1][1], pred_txt, text])
        else:
    ...
    def encode_img(self, img_list):
        ...
        image_emb, _, preds_txt, pred_cls = self.model.encode_img(image)
        img_list.append(image_emb)
        return preds_txt, pred_cls
