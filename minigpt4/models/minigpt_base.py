...
class MiniGPTBase(BaseModel):
    ...
    def __init__(
        ...
        self.visual_encoder, self.ln_vision, self.cls_head = self.init_vision_encoder(
    ...
    def preparing_embedding(self, samples):
        ...
        if 'image' in samples:
            # img_embeds, img_atts = self.encode_img(samples["image"])  # default
            img_embeds, img_atts, preds_txt, preds_clss = self.encode_img(samples["image"])  # use preds_txt to prompt LLAMA
        else:
        ...
        if 'conv_q' in samples:
            ...
            if "instruction_input" in samples:
                instruction = samples["instruction_input"] # default
                prompt_instruction = []
                system_instruction = 'You are a cytopathologist. Please analyze cervical cytology images according to the 2014 Bethesda System (TBS). '
                
                for i in range(len(instruction)):  # 遍历 batch_size
                    single_instruction = instruction[i]  # 获取第 i 个 instruction
                    formatted_instruction = system_instruction + self.prompt_template.format(single_instruction.format(preds_txt[i], preds_clss[i]))  # 格式化 instruction
                    # print(f"Formatted instruction {i}: {formatted_instruction}")
                    prompt_instruction.append(formatted_instruction)  # 将结果添加到列表中
                instruction = prompt_instruction  # 重新赋值 instruction
